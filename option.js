...
PageManager.prototype.information = function() {
    ...
    history += '\nver.2.55 返信ボタンが機能しなくなった問題を修正しました\nver.2.56 投稿のアクティビティの非表示がうまく機能していなかった問題を修正しました。通知の各ポストに返信ボタンが表示されなくなっていた問題を修正しました。リップルボタンを表示可能にしました(リップルは設定画面から、初期化＞ボタンを初期化する から初期化することで使用可能になります)\nver.2.57 ミュートボタンを追加しました(設定画面から、初期化＞ボタンを初期化するから初期化することで使用可能になります)\nver.2.58 ポストへの返信ボタンを有効にすると通知欄でコメントを展開しにくくなる問題を修正しました\nver.2.59 通知画面でシフトエンター&返信ボタンが効かなくなっていた問題の修正を行いました\nver.2.60 返信ボタンが表示されない場合がある問題を修正しました\nver.2.61 投稿時にシフトエンターによる投稿がうまくいかない仕様を補う機能を追加しました\nver.2.62 返信ボタンが正常に動作していない問題を修正しました。Shift+Enterによる投稿の高速化を行いました。\nver.2.63 バグ修正\nver.2.64 バグ修正\nver.2.65 Twitterの投稿時のデフォルト文字列を変更しました。（ボタンの初期化で有効になります）\n';
    history += 'ver.2.66 正常にボタンが機能していなかった問題の修正・マニフェストバージョン2に対応・ミュート機能が動かないバグを修正';
